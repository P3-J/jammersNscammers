...
public partial class worldcontroller : Node2D
{
    ...
    [Export]
    PackedScene objectIceBall;

    [Export]
    PackedScene objectSkiMan;

    [Export]
    PackedScene objectLagoon;

    [Export]
    PackedScene objectBird;

    [Export]
    PackedScene objectSnowflake;

    [Export]
    PackedScene objectRocket;
    ...
    private void spawnHillObjects()
    {
        ...
        for (int i = 1; i < objCount; i++)
        {
            ...
            Vector2 objPosition = new(
                distancebetween,
                -(distancebetween * ratio) + randomizedYdiff
            );
            obj.GlobalPosition = objPosition;

            // randomly spawn rocket object above other object
            if (rand.Next(1, 10) == 1)
            {
                Area2D rocket = objectRocket.Instantiate<Area2D>();
                GetTree().CurrentScene.AddChild(rocket);
                rocket.GlobalPosition = new Vector2(objPosition.X, objPosition.Y - 200f);
            }
        }
    }
    ...
    private void SpawnStuffThatFallsDown()
    {
        ...
        rigidObj.LinearVelocity = new Vector2(
            rigidObj.LinearVelocity.X - 100,
            rigidObj.LinearVelocity.Y
        );
